...
/**
 * Sample HTTP Server (like a mini tomcat, runs at port 10080 will run forever until being killed)
 * 
 * Accept APIs 1. POST: /submitJob; 2. GET: /status/$JOB_ID ; 3. GET /testHeaders to
 * log "sample" keyvalue in header
 * 
 * note that the job map will never be clean up until it reaches 8192
 * 
 * if request is submit a job: 1. submit job POST: /submitJob return: {"status":
 * "/status/01218499-a5fe-47cf-a0a8-8e9b106c5219", "progress": 0}
 * 
 ...
 *
 */
public class HttpServerSampleAsyncApiWithPollableJob extends NanoHTTPD {

    public static String AUTH_KEY = "SAMPLE_AUTH_KEY";
    private static Logger logger = Logger
            .getLogger(HttpServerSampleAsyncApiWithPollableJob.class.getName());
    public static final int PORT = 10080;

    public HttpServerSampleAsyncApiWithPollableJob() throws IOException {
        super(PORT);
    ...
    public static void main(String[] args) {
        try {
            new HttpServerSampleAsyncApiWithPollableJob();
            while (true) {
                ;
            }
